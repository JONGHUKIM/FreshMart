...
public class StorageUpdateFrame extends JFrame {
	...
	public void initialize() {
		...
		btnUpdate.setBounds(236, 160, 100, 30);
		btnUpdate.addActionListener(e -> saveFoodItem(categoryComboBox_1));
		contentPane.add(btnUpdate);

		btnCancel = new JButton("취소");
		btnCancel.setFont(new Font("맑은 고딕", Font.BOLD, 15));
		btnCancel.setBounds(348, 160, 100, 30);
	...
	private void saveFoodItem(JComboBox<String> categoryComboBox) {
		...
		try {
			...
			if (result == 1) {
				JOptionPane.showMessageDialog(this, "수정되었습니다.", "정보", JOptionPane.INFORMATION_MESSAGE);
				dispose();
			} else {
