...
public class FreezerStorage extends JFrame {
	...
	public void initialize() {
		...
		if (!list.isEmpty()) {
			...
			if (daysRemaining > 0) {
				...
				expirationDateLabel.setForeground(Color.RED);
				JOptionPane.showMessageDialog(FreezerStorage.this, "오늘이 유통기한 마지막인 식품이 있습니다!", "경고",
		...
		btnUpdate.addActionListener(e -> {
			...
			if (selectedRow != -1) {
				...
				Component parentComponent = FreezerStorage.this;
		...
		btnDelete.addActionListener(e -> deleteFreezer());
	...
	private void deleteFreezer() {

		int index = table.getSelectedRow();
		if (index == -1) {
			JOptionPane.showMessageDialog(FreezerStorage.this, "테이블에서 삭제할 행을 먼저 선택하세요.", "경고",
					JOptionPane.WARNING_MESSAGE);
			return;
		}

		int confirm = JOptionPane.showConfirmDialog(FreezerStorage.this, "정말 삭제할까요?", "삭제 확인",
		...
		if (confirm == JOptionPane.YES_OPTION) {
			...
			if (result == 1) {
				initializeTable();
				JOptionPane.showMessageDialog(FreezerStorage.this, "삭제 성공");
			} else {
				JOptionPane.showMessageDialog(FreezerStorage.this, "삭제 실패");
			}
		}
